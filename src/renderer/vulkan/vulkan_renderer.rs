...
impl VulkanRenderer{
  pub fn new( window: Arc<Window>, queue: Arc<Queue> ) -> Self{
    ...
    VulkanRenderer { window, queue, swapchain, framebuffers, render_pass, last_render, skia_ctx, swapchain_is_valid, last_invalidation: Utc::now().timestamp_millis() }
  }

  pub fn invalidate_swapchain( &mut self ){
    let now = Utc::now().timestamp_millis();
    if now - self.last_invalidation < 50 { return; } // If we recreate the swapchain some GPU drivers freak out.
